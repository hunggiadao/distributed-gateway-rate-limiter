    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            ...
            try {
                ...
                Process gatewayProcess = new ProcessBuilder(javaBin, "-cp", classpath, className, "gateway").start();
                
                // CRITICAL FIX: Give the Gateway JVM 2 full seconds to boot and bind the TCP port
                Thread.sleep(2000);

                // 2. Spawn isolated client cluster dashboard processes stack-aligned as requested
                ...
                // 3. WATCHDOG LOOP: Monitor the Gateway process and resurrect it if it dies
                while (true) {
                    int exitCode = gatewayProcess.waitFor();
                    System.out.println("Gateway process terminated (Code: " + exitCode + "). Restarting a new instance...");
                    
                    // Spin up a fresh gateway process to take its place
                    gatewayProcess = new ProcessBuilder(javaBin, "-cp", classpath, className, "gateway").start();
                }
